...
function clob_form_urlencoded (p_clob in clob) return clob
as
  l_pos         pls_integer := 1;
  l_lob_len     integer := dbms_lob.getlength (p_clob);
  l_buffer      clob;
...
begin
  ...
  Purpose:       encode clob using x-www-form-urlencoded encoding
 
  Remarks:       based on https://docs.oracle.com/cd/B28359_01/appdev.111/b28419/u_url.htm
 
  Who     Date        Description
  ------  ----------  --------------------------------
  JHI     24.07.2016  Created
 
  */
  
  if p_clob is not null then
  
    l_chunk_size := dbms_lob.getchunksize(p_clob);
    
    dbms_lob.createtemporary (l_returnvalue, true);
    dbms_lob.open (l_returnvalue, dbms_lob.lob_readwrite);
    
    while (l_pos < l_lob_len) loop
      l_buffer := dbms_lob.substr (p_clob, l_chunk_size, l_pos);
      l_buffer := utl_url.escape(l_buffer, true);
      dbms_lob.writeappend (l_returnvalue, dbms_lob.getlength(l_buffer), l_buffer);
...
end clob_form_urlencoded;


function escape_json_string (p_clob in clob) return clob
as
  l_pos            pls_integer := 1;
  l_buffer         varchar2 (32767);
  l_lob_len        integer := dbms_lob.getlength (p_clob);
  l_chunk_size          pls_integer := (76 / 4 * 3)-9;
  l_returnvalue    clob;
begin
 
  /*
 
  Purpose:      encode json string
  ...
  */
  ...
  if p_clob is not null then
    ...
    while (l_pos < l_lob_len) loop
      l_buffer := dbms_lob.substr (p_clob, l_chunk_size, l_pos);
      l_buffer := apex_escape.js_literal(l_buffer, p_quote => null);
      dbms_lob.writeappend (l_returnvalue, length (l_buffer), l_buffer);
      l_pos := l_pos + l_chunk_size;
    end loop;
